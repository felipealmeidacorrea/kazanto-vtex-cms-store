...
var codeFunctionsStore = {
    ...
    quantEstoqueShelf: function() {
        $(".vitrine.prateleira ul li .product").each(function(index, element) {
            ...
            vtexjs.catalog.getProductWithVariations(idProd).done(function(product) { var quantProduto = product.skus[0].availablequantity; if (quantProduto < 6 && quantProduto > 1) { $(element).find(".quant-produto").html("Restam apenas " + quantProduto + " unidades").show() } if (quantProduto == 1) { $(element).find(".quant-produto").html("Ãšltima Unidade").show() } })
        })
    },
    init: function() {
