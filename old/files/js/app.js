...
var codeFunctionsStore = {
    ...
    sliderProductsHome: function() {
        ...
        var $sliderSlidesTree = $sliderSectionTree.find(".slider__slides .vitrine > ul");
            ...
            if (emailUsuario == null || emailUsuario == "" || !regex.test(emailUsuario)) {
                ...
                $.ajax({
                    ...
                    success: function(resp) {
                        ...
                        $this.find(".subscribe--news").append('<div class="subscribe__controls subscribe_sucess">Obrigado por se cadastrar na Kazanto. <br/> Em breve entraremos em contato oferecendo nossas melhores promoções!</div>')
                    }
                })
            }
        })
    ...
    menuFiltro: function() {
        $(".search-single-navigator").prepend('</div> <div class="filtro-topo"> <img src="/arquivos/filter-icon.svg" > Filtro<span></span></div>');
        $(".orderBy").prepend('<img src="/arquivos/filter-button.png" >');
        $(".orderBy img").on("click", function() {
            $(".search-single-navigator").toggleClass("open");
            $(".vitrine.resultItemsWrapper").prepend('<div class="background"></div>')
        });
    ...
    getTitleBannerBrands: function() {
        $(".section__content__boxes__brands .box_brand_selection").each(function() {
        ...
        $(".vitrine.prateleira ul li .product").each(function(index, element) {
            ...
            vtexjs.catalog.getProductWithVariations(idProd).done(function(product) {
                ...
                if (quantProduto < 6 && quantProduto > 1) {
                    $(element).find(".quant-produto").html("Restam apenas " + quantProduto + " unidades").show()
                }
                if (quantProduto == 1) {
                    $(element).find(".quant-produto").html("Última Unidade").show()
                }
            })
        })
    },
